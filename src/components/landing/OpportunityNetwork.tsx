...
const typeStyles: Record<string, string> = {
  procura: "bg-connection/15 text-connection",
  oferece: "bg-primary-light/15 text-primary-light",
  contratando: "bg-white/15 text-white",
  parceria: "bg-primary/20 text-primary-light",
};

export function OpportunityNetwork({ opportunities }: OpportunityNetworkProps) {
  return (
    <section id="oportunidades" className="bg-surface-dark px-6 py-28 text-foreground-dark">
      <div className="mx-auto max-w-6xl">
        ...
        >
          {opportunities.map((opportunity) => (
            <div
              key={opportunity.id}
              className="glass-card-dark w-[280px] shrink-0 snap-start rounded-2xl p-6"
            >
              <span
                className={`inline-block rounded-full px-2.5 py-1 text-[11px] font-medium ${
                  typeStyles[opportunity.type] ?? "bg-white/10 text-white/70"
                }`}
              >
              ...
              <p className="mt-2 text-[13px] leading-relaxed text-white/55">{opportunity.description}</p>
              ...
              <div className="mt-5 flex items-center gap-2 border-t border-white/10 pt-4">
                ...
                <p className="truncate text-[12px] text-white/60">{opportunity.business.name}</p>
              </div>
              ...
              <a
                ...
                className="mt-4 inline-flex items-center gap-1.5 text-[12px] font-medium text-connection"
              >
