...
export function SmartSearch({ businesses }: SmartSearchProps) {
  ...
  return (
    <section className="bg-surface-dark px-6 py-28 text-foreground-dark">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <FadeUp delay={0.1} className="mx-auto mt-10">
          <form
            ...
            className="glass-dark flex items-center gap-3 rounded-full p-2 pl-6"
          >
            <svg viewBox="0 0 20 20" fill="none" className="h-5 w-5 shrink-0 text-white/40">
            ...
            <input
              ...
              className="min-w-0 flex-1 bg-transparent py-3 text-[15px] text-white placeholder:text-white/40 focus:outline-none"
            />
            <button
              type="submit"
              className="shrink-0 rounded-full bg-white px-5 py-3 text-[14px] font-medium text-black transition-opacity hover:opacity-85"
            >
              Buscar
            </button>
          </form>
        ...
        <FadeUp delay={0.15} className="mt-6 flex flex-wrap justify-center gap-2">
          {suggestions.map((suggestion) => (
            <button
              ...
              className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-[13px] text-white/60 transition-colors hover:border-white/30 hover:text-white"
            >
              {suggestion.label}
            </button>
          ))}
        ...
        {results.length > 0 && (
          ...
          >
            {results.map((business) => (
              <div key={business.id} className="glass-card-dark rounded-xl p-4">
                <p className="text-[14px] font-semibold tracking-tight">{business.name}</p>
                ...
                <a
                  ...
                  className="mt-2 inline-flex items-center gap-1.5 text-[12px] font-medium text-connection"
                >
                  Falar no WhatsApp
        ...
        {(results.length > 0 || query) && query !== "__oportunidades__" && (
          <a
            ...
            className="mt-8 inline-block text-[13px] font-medium text-connection hover:underline"
          >
