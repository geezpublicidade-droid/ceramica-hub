      ...
      <div className="mx-auto max-w-6xl">
        ...
        >
          <div className="flex items-center gap-1.5 px-2 py-2">
            ...
            <span className="h-2.5 w-2.5 rounded-full bg-foreground/15" />
            <span className="h-2.5 w-2.5 rounded-full bg-foreground/15" />
            <span className="ml-3 text-[14px] text-muted">ceramicahub.com.br</span>
          </div>

          <div className="rounded-2xl border border-border bg-white/60 p-5 sm:p-6">
            <div className="flex items-center gap-3 rounded-full border border-border bg-white px-5 py-3 text-[16px] text-muted">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {categories.slice(0, 5).map((category) => (
                <span
                  key={category}
                  className="rounded-full border border-border bg-white px-3.5 py-1.5 text-[14px] text-muted"
                >
            ...
            <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
              {preview.map((business) => (
                ...
                >
                  ...
                  <p className="mt-3 text-[16px] font-semibold tracking-tight">{business.name}</p>
                  <p className="mt-0.5 text-[14px] text-muted">{tCategories(business.category)}</p>
                  <a
