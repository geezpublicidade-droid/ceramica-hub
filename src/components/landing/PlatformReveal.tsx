    ...
    <section className="bg-surface-dark px-6 py-28 text-foreground-dark">
      <div className="mx-auto max-w-6xl">
        <FadeUp className="max-w-2xl">
          <p className="text-[13px] font-medium uppercase tracking-[0.2em] text-connection">
            A plataforma
          </p>
        ...
        <FadeUp
          delay={0.1}
          className="glass-dark relative mt-14 overflow-hidden rounded-3xl p-3 shadow-[0_40px_100px_-30px_rgba(0,0,0,0.6)] sm:p-4"
        >
          <div className="flex items-center gap-1.5 px-2 py-2">
            <span className="h-2.5 w-2.5 rounded-full bg-white/20" />
            <span className="h-2.5 w-2.5 rounded-full bg-white/20" />
            <span className="h-2.5 w-2.5 rounded-full bg-white/20" />
            <span className="ml-3 text-[12px] text-white/40">ceramicahub.com.br</span>
          </div>

          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
            <div className="flex items-center gap-3 rounded-full border border-white/15 bg-white/5 px-5 py-3 text-[14px] text-white/50">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {categories.slice(0, 5).map((category) => (
                <span
                  key={category}
                  className="rounded-full border border-white/10 bg-white/5 px-3.5 py-1.5 text-[12px] text-white/60"
                >
                  {category}
                </span>
              ))}
            ...
            <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
              {preview.map((business) => (
                ...
                >
                  ...
                  <p className="mt-0.5 text-[12px] text-white/45">{business.category}</p>
                  <a
                    href={buildWhatsAppLink(business.phone, business.name)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-3 inline-flex items-center gap-1.5 text-[12px] font-medium text-connection"
                  >
                    Falar no WhatsApp
