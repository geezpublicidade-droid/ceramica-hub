 ...
 *
 * Adaptado do componente original: sem injeção de fonte remota (CDN de
 * terceiros) -- herda a tipografia do próprio site (Montserrat) em vez de
 * carregar Geist via <style> inline.
 */
export function SqueezeCarousel({
  ...
  const ids = useId();
  // Começa em `visible`, não 0: as `visible` chaves iniciais já usam 0..visible-1
  // (ver window0 abaixo) sem tocar no ref durante o render -- só daí em diante
  // `seed.current++` roda dentro de `step`, que é sempre um event handler.
          ...
          return (
            ...
            >
              <p className="max-w-[46rem] text-[15px] leading-[1.6] text-balance @lg:text-[17px]">
                ...
                {slide.description && <span className="text-muted-foreground">{slide.description}</span>}
              </p>
