...
export type ProfileCompletenessMap = Map<string, string[]>;

/** businessId -> lista do que falta (vazio = perfil completo). Reusado pelo
 * contador agregado do dashboard e pelo indicador por empresa na lista de
 * Aprovadas -- mesma regra, um lugar só. */
export async function getProfileCompletenessMap(): Promise<ProfileCompletenessMap> {
  const supabase = createServiceClient();
  const [businessesResult, servicesResult, photosResult] = await Promise.all([
  ...
  const map: ProfileCompletenessMap = new Map();
  for (const business of businessesResult.data ?? []) {
    const missing: string[] = [];
    if (!business.description?.trim()) missing.push("descrição");
    if (!servicedIds.has(business.id)) missing.push("serviço");
    if (!photographedIds.has(business.id)) missing.push("foto");
    map.set(business.id, missing);
  }
  return map;
}
...
export async function getAdminDashboardStats(): Promise<AdminDashboardStats> {
  ...
  const [signupsResult, campaignsResult, searchesResult, impressionsResult, clicksResult, opportunities, benefits, completeness] =
    await Promise.all([
  ...
  const incompleteProfiles = Array.from(completeness.values()).filter((missing) => missing.length > 0).length;
