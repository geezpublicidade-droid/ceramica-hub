...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = await getTranslations("PoliticaCadastro");
  const title = t("metaTitle");
  const description = t("metaDescription");
  return {
    title,
    description,
    alternates: { canonical: "/politica-de-cadastro" },
    ...buildSocialMetadata({ title, description, locale, path: "/politica-de-cadastro" }),
  };
}

export default async function PoliticaCadastroPage() {
  const t = await getTranslations("PoliticaCadastro");
